#!/usr/bin/env python3
# P81_ROBUST_FEEDBACK_MONITOR
"""Monitor TARAGAY-T1 P81 dual-ADC robust-feedback DO-NOT-FLY bench firmware.

P81 keeps the $TGY68 diagnostic stream, hard-locks needle motor output OFF,
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P81 UART robust feedback diagnostic | port={port} baud={baud} fields={len(FIELDS)}")
...
def compact_status(f: dict[str, str]) -> str:
    ...
    flags = iv("p81_diag_flags")
    a1 = iv("p81_adc1_raw")
    a2 = iv("p81_adc2_raw")
    diff = iv("p81_pair_diff")
    pair = iv("p81_pair_candidate")
    med = iv("p81_median7")
    filt = iv("p81_filtered_adc")
    valid = iv("p81_feedback_valid")
    conf = iv("p81_confidence_pct")
    pairrej = iv("p81_pair_reject_count")
    spikerej = iv("p81_spike_reject_count")
    jumps = iv("p81_confirmed_jump_count")
    holds = iv("p81_hold_count")
    tout = iv("p81_adc_timeout_count")
    rawpp = iv("p81_win_raw_pp")
    filpp = iv("p81_win_filtered_pp")
    rawstep = iv("p81_win_raw_max_step")
    filstep = iv("p81_win_filtered_max_step")
    vrefpp = iv("p81_vref_win_pp_raw12")
    samples = iv("p81_samples_total")
    en = iv("needle_enabled")
    rpwm = iv("needle_rpwm")
    lpwm = iv("needle_lpwm")

    labels = []
    if flags & 0x01: labels.append("PAIR_REJECT")
    if flags & 0x02: labels.append("SPIKE_HELD")
    if flags & 0x04: labels.append("JUMP_OK")
    if flags & 0x08: labels.append("INVALID")
    if flags & 0x10: labels.append("LOW_CONF")
    if flags & 0x20: labels.append("VREF_MOVE")
    if flags & 0x40: labels.append("TIMEOUT")
    if flags & 0x80: labels.append("WARMUP")
    ...
    elif rawpp >= 30 and filpp <= 10 and conf >= 60:
    ...
    elif valid and conf >= 80 and filpp <= 6:
        hint = "robust feedback bu pencerede guvenilir gorunuyor"
    else:
        hint = "20-30 s dokunmadan kayit al; raw-vs-filtered farkini biriktir"

    return (
        f"t={iv('time_ms')/1000.0:7.2f}s | SYS={f.get('system_ok')}/{f.get('system_fault')} "
        f"flight={f.get('flight_active')} | P81 flags=0x{flags:02X}[{flag_text}] | "
        f"A1/A2={a1:4d}/{a2:4d} d={diff:2d} pair={pair:4d} med={med:4d} FILT={filt:4d} | "
        f"VALID={valid} conf={conf:3d}% | win raw/filt pp={rawpp}/{filpp} "
        f"step={rawstep}/{filstep} | reject pair/spike={pairrej}/{spikerej} "
        f"jumpOK={jumps} hold={holds} | vrefPP={vrefpp} timeout={tout} samples={samples} | "
...
def main() -> int:
    ...
    parser.add_argument("--log", default="uart_p81_robust_feedback.txt",
                        help="decoded UART log file (default: uart_p81_robust_feedback.txt)")
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P81 UART robust feedback diagnostic | port={selected_port} "
                ...
                if clean.startswith("$TGY67,") or clean.startswith("$TGY66,") or clean.startswith("$TGY65,") or clean.startswith("$TGY64,") or clean.startswith("$TGY63,") or clean.startswith("$TGY62,"):
                    ...
                    if previous_seen == 1 or (previous_seen % 10) == 0:
                        print(
                            ...
                            "Eski firmware olabilir; P81 $TGY68 frame kullanmalidir.",
                            flush=True,
                        )
