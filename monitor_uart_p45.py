#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P45 $TGY56 UART telemetry (228 fields)."""
...
FIELDS = (
    ...
    'eskf_cov_diag_min_u1e6', 'eskf_cov_diag_max_m1e3', 'eskf_cov_sym_max_u1e6', 'baro_fresh', 'lidar_fresh', 'vertical_source_mask',
...
def decode_frame(line: str) -> dict[str, str]:
    line = line.strip()
    if not line.startswith("$TGY56,"):
        raise ValueError("not a $TGY56 frame")
    if "*" not in line:
...
def serial_lines(port: str, baud: int) -> Iterable[str]:
    ...
    with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
        print(f"# P45 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
        while True:
