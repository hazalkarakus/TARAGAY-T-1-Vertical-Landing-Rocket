...
def serial_lines(port,baud,dur):
 ...
 with serial.Serial(port,baudrate=baud,timeout=.5) as s:
  ...
  print('='*86)
  print('P112R12R1 PRODUCTION AUTONOMOUS 950+ / SAME-TARGET GUARD / INERT - READ ONLY UART')
...
def run(a):
 if a.capture: lines=file_lines(a.capture); log=None
  ...
  log.write(f'# P112R12R1 production autonomous 950+ same-target guard inert | port={a.port} baud={a.baud}\n')
...
def main():
 p=argparse.ArgumentParser();p.add_argument('capture',nargs='?');p.add_argument('--port',default='COM21');p.add_argument('--baud',type=int,default=BAUD);p.add_argument('--duration',type=float,default=40);p.add_argument('--log',default='uart_p112r12r1_prod_auto_950plus_inert.txt');return run(p.parse_args())
