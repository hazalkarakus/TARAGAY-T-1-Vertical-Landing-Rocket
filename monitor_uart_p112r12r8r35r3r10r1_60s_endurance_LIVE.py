...
def main():
    ...
    ap.add_argument('--out',default='uart_p112r12r8r35r3r10r1_60s_endurance.txt')
    a=ap.parse_args()
    import serial
    rows=[]; rejected=0; last_fail_count=0
    t0=time.monotonic(); out=Path(a.out)
    print('R8R35R3R10R1 60S ENDURANCE - 30HZ GROUND / 7.5HZ FLIGHT RAM')
    print('BASINCISIZ/INERT: READY=1 bekle, PE9 ayir; needle motor gucu BAGLI olsun.')
    print('PE9 sonrasi SDIO yeni komut YOK; flight RAM capture 7.5 Hz. Kapasite ~68.3 s. 60 s testten sonra E-STOP + 1 s quiet -> SD replay.')
    ...
    print('\n================ R8R35R3R10R1 60S ENDURANCE OZET =================')
