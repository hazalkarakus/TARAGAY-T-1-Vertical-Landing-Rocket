#!/usr/bin/env python3
"""TARAGAY-T1 R8R35R3R10R1 60 s endurance monitor."""
...
def main():
    ...
    ap.add_argument('--out',default='uart_p112r12r8r35r3r10r2_estop_fb_recovery.txt')
    a=ap.parse_args()
    import serial
    rows=[]; rejected=0; last_fail_count=0
    t0=time.monotonic(); out=Path(a.out)
    print('R8R35R3R10R2 ESTOP + FLIGHT FEEDBACK RECOVERY - 60S RAM')
    print('BASINCISIZ/INERT: READY=1 bekle, PE9 ayir; needle motor gucu BAGLI olsun.')
    print('PE9 sonrasi SDIO yeni komut YOK; flight RAM 7.5 Hz. 45-60 s testte needle hareketini izle, sonra E-STOP ver. STOP latched olmali ve needle CLOSED ref civarina donmeli; sonra RAM->SD replay.')
    ...
    with serial.Serial(a.port,baudrate=a.baud,timeout=.25) as ser, out.open('w',encoding='utf-8') as fp:
        ...
        while a.duration<=0 or time.monotonic()-t0<a.duration:
            ...
            print(
                ...
                f"STOP={iv(f,'stop_latched')} EClose={iv(f,'estop_close_active')}/{iv(f,'estop_close_complete')}/{iv(f,'estop_close_failed')} "
    ...
    print('\n================ R8R35R3R10R2 ESTOP/FB RECOVERY OZET =================')
