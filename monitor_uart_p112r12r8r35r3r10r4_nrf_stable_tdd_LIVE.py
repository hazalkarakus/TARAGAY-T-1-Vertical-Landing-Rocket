#!/usr/bin/env python3
"""TARAGAY-T1 R8R35R3R10R4 nRF stable-TDD inert monitor."""
...
def main():
    ...
    ap.add_argument('--out',default='uart_p112r12r8r35r3r10r4_nrf_stable_tdd.txt')
    a=ap.parse_args()
    import serial
    rows=[]; rejected=0; last_fail_count=0
    t0=time.monotonic(); out=Path(a.out)
    print('R8R35R3R10R4 NRF STABLE TDD - ONE FAST DOWNLINK PER COMMAND IN FLIGHT')
    print('BASINCISIZ/INERT: READY=1 bekle, PE9 ayir; needle motor gucu BAGLI olsun.')
    print('PE9 sonrasi: ground 20 Hz command heartbeat devam etmeli; rocket her kabul edilen komuta TEK FAST downlink donmeli (duplicate/detail yok). nRF_link kesintisiz 1, age tercihen <150 ms. Test sonunda E-STOP ver.')
    ...
    print('\n================ R8R35R3R10R4 NRF STABLE TDD OZET =================')
