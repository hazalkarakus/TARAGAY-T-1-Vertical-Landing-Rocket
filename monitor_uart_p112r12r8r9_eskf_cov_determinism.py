#!/usr/bin/env python3
"""TARAGAY-T1 P112R12R8R9 ESKF deterministic-aiding observer — READ ONLY.

Firmware emits compact 87-field $TGY70 timing telemetry. It separates pure ESKF
...
"scheduler_realigns","bg_uart_us","bg_uart_max_us","uart_defers","sd_ready","p110_pwm","p111_moves","p111_fault","needle_fault","rcs_mask","hardoff","isr_max_us")
...
def dec(line):
    s=line.strip()
    if not s.startswith(PREFIX) or "*" not in s: raise ValueError("not TGY70")
...
def serial_lines(port,baud,duration):
    ...
    with serial.Serial(port,baudrate=baud,timeout=.5) as s:
        ...
        print("="*118)
        print("P112R12R8R9 ESKF DETERMINISM / COMPACT TGY70 / READ ONLY")
...
def status(f,tag=""):
    ...
    return (f"t={iv(f,'time_ms')/1000:7.2f}s {tag:>10} | "
            ...
            f"miss I/B/L/E={iv(f,'miss_imu')}/{iv(f,'miss_baro')}/{iv(f,'miss_lidar')}/{iv(f,'miss_eskf')} SYS={iv(f,'system_ok')}/{iv(f,'system_fault')}")
...
def run(a):
    ...
    if log: log.write(f"# P112R12R8R9 ESKF determinism | port={a.port} baud={a.baud}\n")
    ...
    try:
        ...
        if log:log.close()
    if latest is None: print("TGY70 frame yok"); return 2
    ...
    pass_ok=(dur>=90.0 and baro_good and not baro_drops and lidar_good and not lidar_drops and
             ...
             max_corr<700 and max_pred<500 and max_task_eskf<900 and max_cov<650 and max_uart<900 and
    ...
    if pass_ok:
        print("\nSONUC: PASS - R8R9 ESKF DETERMINISTIC AIDING INERT CANDIDATE")
        return 0
    print("\nSONUC: PASS DEGIL - TXT LOGU GONDER")
    return 1

def main():
    ap=argparse.ArgumentParser(); ap.add_argument("capture",nargs="?"); ap.add_argument("--port",default="COM21"); ap.add_argument("--baud",type=int,default=BAUD); ap.add_argument("--duration",type=float,default=120.0); ap.add_argument("--print-every",type=int,default=10); ap.add_argument("--log",default="uart_p112r12r8r9_eskf_cov_determinism.txt"); a=ap.parse_args(); raise SystemExit(run(a))
