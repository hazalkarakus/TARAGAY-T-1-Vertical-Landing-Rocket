#!/usr/bin/env python3
# P112_DETERMINISTIC_ADAPTIVE_MONITOR
"""Monitor TARAGAY-T1 P112 deterministic UART-independent adaptive needle commissioning."""
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P112R1 ADC-serialized deterministic adaptive diagnostic | port={port} baud={baud} fields={len(FIELDS)}")
...
def compact_status(f: dict[str, str]) -> str:
    ...
    sup_state = iv("p111_state")
    sup_result = iv("p111_result")
    cycle = iv("p111_cycle")
    completed = iv("p111_completed")
    pass_mask = iv("p111_pass_mask")
    phase = iv("p111_phase")
    moves = iv("p111_moves_completed")
    baseline = iv("p111_baseline_adc")
    open_target = iv("p111_open_target_adc")
    lopen = iv("p111_learned_open_pwm")
    lclose = iv("p111_learned_close_pwm")
    copen = iv("p111_learned_open_coast")
    cclose = iv("p111_learned_close_coast")
    lo_err = iv("p111_last_open_error_adc")
    lc_err = iv("p111_last_close_error_adc")
    lo_pwr = iv("p111_last_open_powered_ms")
    lc_pwr = iv("p111_last_close_powered_ms")
    lo_bo = iv("p111_last_open_breakaway_pwm")
    lc_bo = iv("p111_last_close_breakaway_pwm")
    lo_stop = iv("p111_last_open_stop_adc")
    lc_stop = iv("p111_last_close_stop_adc")
    lo_coast = iv("p111_last_open_coast_adc")
    lc_coast = iv("p111_last_close_coast_adc")
    sup_abort = iv("p111_abort_reason")

    state = iv("p110_state")
    result = iv("p110_result")
    direction = iv("p110_direction")
    current = iv("p110_current_adc")
    target = iv("p110_target_adc")
    pwm = iv("p110_active_pwm")
    learned = iv("p110_learned_breakaway_pwm")
    sustain = iv("p110_sustain_pwm")
    speed = iv("p110_speed_adc_s")
    stopd = iv("p110_stop_distance_adc")
    coast = iv("p110_coast_max_adc")
    ferr = iv("p110_final_error_adc")
    powered_ms = iv("p110_total_powered_ms")
    corr = iv("p110_correction_count")
    ...
    sup_state_name = {0:"WAIT",1:"OPEN_MOVE",2:"DWELL_CLOSE",3:"CLOSE_MOVE",4:"DWELL_NEXT",5:"DONE",6:"ABORT"}.get(sup_state, f"S{sup_state}")
    sup_result_name = {0:"RUNNING",1:"PASS",2:"ABORT",3:"START_RANGE"}.get(sup_result, f"R{sup_result}")
    p110_state_name = {0:"WAIT",1:"SEARCH",2:"DRIVE",3:"BRAKE",4:"CORR_DWELL",5:"DONE"}.get(state, f"S{state}")
    p110_result_name = {0:"RUN",1:"PASS",2:"ABORT",3:"OVERSHOOT",4:"NO_BO",5:"PWR_TO"}.get(result, f"R{result}")
    dir_name = "OPEN" if direction == 0 else "CLOSE"
    phase_name = "OPEN" if phase == 0 else "CLOSE"
    mode_name = {0:"BOOT",1:"NORMAL",2:"QUAR",3:"TRAJ"}.get(mode, f"M{mode}")
    abort_name = P99_ABORT_NAMES.get(sup_abort, f"ABORT_{sup_abort}")

    if sup_result == 1:
        hint = "5/5 adaptive round trips PASS"
    elif sup_state == 6:
        hint = f"commissioning stopped: {abort_name}"
    elif sup_state in (1, 3):
        hint = "P110 is learning this direction online"
    else:
        hint = "bridge-off learning dwell"

    return (
        f"t={iv('time_ms')/1000.0:7.2f}s | P112/P111 {sup_state_name} result={sup_result}:{sup_result_name} "
        f"C={cycle}/5 done={completed} moves={moves}/10 mask=0x{pass_mask:02X} phase={phase_name} | "
        f"base/openT={baseline}/{open_target} learnedPWM O/C={lopen}/{lclose} coast O/C={copen}/{cclose} | "
        f"last O err/pwr/BO/stop/coast={lo_err:+d}/{lo_pwr}/{lo_bo}/{lo_stop}/{lo_coast} "
        f"C={lc_err:+d}/{lc_pwr}/{lc_bo}/{lc_stop}/{lc_coast} | "
        f"P110 {p110_state_name} {dir_name} {p110_result_name} now/T={current}/{target} PWM={pwm} BO/sus={learned}/{sustain} "
        f"v={speed} stop={stopd} coast={coast} ferr={ferr:+d} pwr={powered_ms} corr={corr} | "
        f"P83 V={valid} {mode_name} conf={conf}% rej={pairrej}/{raterej}/{qcount} | {hint}"
    )
...
def main() -> int:
    ...
    parser.add_argument("--log", default="uart_p111_5cycle_adaptive_learning.txt",
                        help="decoded UART log file (default: uart_p111_5cycle_adaptive_learning.txt)")
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P112R1 ADC-serialized deterministic adaptive diagnostic | port={selected_port} "
                ...
                if clean.startswith("$TGY67,") or clean.startswith("$TGY66,") or clean.startswith("$TGY65,") or clean.startswith("$TGY64,") or clean.startswith("$TGY63,") or clean.startswith("$TGY62,"):
                    ...
                    if previous_seen == 1 or (previous_seen % 10) == 0:
                        print(
                            ...
                            "Eski firmware olabilir; P112 $TGY68 frame kullanmalidir.",
                            flush=True,
                        )
                ...
                if clean.startswith("$TGY61,") or clean.startswith("$TGY60,") or clean.startswith("$TGY59,") or clean.startswith("$TGY58,") or clean.startswith("$TGY57,") or clean.startswith("$TGY56,"):
                    ...
                    if older_seen == 1:
                        print(
                            ...
                            "P112 flash durumunu kontrol et.",
                            flush=True,
                        )
                    continue
