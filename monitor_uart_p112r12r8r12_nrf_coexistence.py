#!/usr/bin/env python3
"""TARAGAY-T1 P112R12R8R12 nRF coexistence qualification observer — READ ONLY."""
...
def serial_lines(port,baud,duration):
    ...
    with serial.Serial(port,baudrate=baud,timeout=.5) as s:
        ...
        print("="*120)
        print("P112R12R8R12 NRF COEXISTENCE / COMPACT TGY72 / READ ONLY")
...
def run(a):
    ...
    if log: log.write(f"# P112R12R8R12 nRF coexistence | port={a.port} baud={a.baud}\n")
    ...
    print("\nSONUC:", "PASS - NRF + R8R11 COEXISTENCE" if pass_ok else "PASS DEGIL - TXT LOGU GONDER")
    return 0 if pass_ok else 1

def main():
    ap=argparse.ArgumentParser(); ap.add_argument('capture',nargs='?'); ap.add_argument('--port',default='COM21'); ap.add_argument('--baud',type=int,default=BAUD); ap.add_argument('--duration',type=float,default=120.0); ap.add_argument('--print-every',type=int,default=10); ap.add_argument('--log',default='uart_p112r12r8r12_nrf_coexistence.txt'); a=ap.parse_args(); raise SystemExit(run(a))
