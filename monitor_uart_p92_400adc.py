#!/usr/bin/env python3
# P92_400ADC_MONITOR
"""Monitor TARAGAY-T1 P92 400-ADC closed-loop bench diagnostic (1023 start allowed).
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P87 UART ADC sweep diagnostic | port={port} baud={baud} fields={len(FIELDS)}")
...
def compact_status(f: dict[str, str]) -> str:
    ...
    a1 = iv("p83_adc1_raw")
    a2 = iv("p83_adc2_raw")
    cand = iv("p83_pair_candidate")
    med = iv("p83_median7")
    filt = iv("p83_filtered_adc")
    valid = iv("p83_feedback_valid")
    mode = iv("p83_mode")
    pairdiff = iv("p83_pair_diff")
    ...
    a1min, a1max = iv("p87_adc1_min"), iv("p87_adc1_max")
    a2min, a2max = iv("p87_adc2_min"), iv("p87_adc2_max")
    cmin, cmax = iv("p87_candidate_min"), iv("p87_candidate_max")
    a1step = iv("p87_adc1_max_step")
    a1frm, a1to = iv("p87_adc1_step_from"), iv("p87_adc1_step_to")
    a2step = iv("p87_adc2_max_step")
    a2frm, a2to = iv("p87_adc2_step_from"), iv("p87_adc2_step_to")
    cstep = iv("p87_candidate_max_step")
    cfrm, cto = iv("p87_candidate_step_from"), iv("p87_candidate_step_to")
    fstep = iv("p87_filtered_max_step")
    ffrm, fto = iv("p87_filtered_step_from"), iv("p87_filtered_step_to")
    rawgaps = iv("p87_raw_gap_event_count")
    filtgaps = iv("p87_filtered_gap_event_count")
    pdmax = iv("p87_pair_diff_max")
    sweep = iv("p87_sweep_samples")
    ...
    if en or rpwm or lpwm:
        hint = "DANGER: motor output aktif - gucu kes"
    elif tout:
        hint = "ADC timeout var"
    elif cstep >= 20 and fstep <= 6:
        hint = "RAW buyuk atliyor ama FILTER tutuyor -> analog/raw yolunu incele"
    elif cstep <= 8 and (pairrej or raterej or qcount):
        hint = "RAW surekli; reject/filter esikleri aralik kaybi yapiyor olabilir"
    elif cstep <= 8 and valid:
        hint = "iyi: raw sweep su ana kadar surekli gorunuyor"
    else:
        hint = "potu cok yavas tek yonde cevir; sonra geri don"

    return (
        f"t={iv('time_ms')/1000.0:7.2f}s | A1/A2={a1:4d}/{a2:4d} d={pairdiff:2d} "
        f"CAND={cand:4d} MED={med:4d} FILT={filt:4d} V={valid} {mode_name} | "
        f"RANGE A1={a1min:4d}-{a1max:4d} A2={a2min:4d}-{a2max:4d} C={cmin:4d}-{cmax:4d} | "
        f"MAXSTEP A1={a1step:3d}({a1frm}->{a1to}) A2={a2step:3d}({a2frm}->{a2to}) "
        f"C={cstep:3d}({cfrm}->{cto}) F={fstep:2d}({ffrm}->{fto}) | "
        f"GAP raw/filt={rawgaps}/{filtgaps} pairDmax={pdmax} rej P/R/Q={pairrej}/{raterej}/{qcount} "
        f"samples={sweep} bridge={en}/{rpwm}/{lpwm} | {hint}"
    )
...
def main() -> int:
    ...
    parser.add_argument("--log", default="uart_p91_400adc.txt",
                        help="decoded UART log file (default: uart_p91_400adc.txt)")
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P87 UART ADC sweep diagnostic | port={selected_port} "
                ...
                if clean.startswith("$TGY67,") or clean.startswith("$TGY66,") or clean.startswith("$TGY65,") or clean.startswith("$TGY64,") or clean.startswith("$TGY63,") or clean.startswith("$TGY62,"):
                    ...
                    if previous_seen == 1 or (previous_seen % 10) == 0:
                        print(
                            ...
                            "Eski firmware olabilir; P87 $TGY68 frame kullanmalidir.",
                            flush=True,
                        )
