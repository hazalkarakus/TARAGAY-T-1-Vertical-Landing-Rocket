#!/usr/bin/env python3
"""TARAGAY-T1 P112R12R8R28 five-pose IMU->rocket frame calibration + relay bench observer."""
...
def serial_lines(port,baud,duration):
    ...
    with serial.Serial(port,baudrate=baud,timeout=.5) as s:
        ...
        print("="*110)
        print("R8R28 FIVE-POSE IMU -> ROCKET FRAME CALIBRATION + PHYSICAL RELAY BENCH")
...
def run(a):
        ...
        for line in lines:
            ...
            if ph!=last_phase:
                ...
                if ph==1: print(">>> ROKETI DIK VE SABIT TUT. Yaklasik 2 saniye.")
                    ...
                    print(">>> 5-POZ KALIBRASYON TAMAM. DIK KONUMA DON VE +/-X +/-Y ROLE TESTINI YAP.")
    ...
    print("\nSONUC:","PASS - R8R28 FIVE-POSE CALIBRATION" if pass_ok else "INCELE - calibration/geometry/gate tamamlanmadi")
...
def main():
    ...
    p.add_argument("--log",default="uart_p112r12r8r28_five_pose_imu_rocket_frame_cal.txt");p.add_argument("--capture");p.add_argument("--print-every",type=int,default=3)
