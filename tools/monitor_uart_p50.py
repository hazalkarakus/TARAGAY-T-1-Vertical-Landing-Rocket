#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P50 $TGY61 UART telemetry (258 fields)."""
...
def decode_frame(line: str) -> dict[str, str]:
    line = line.strip()
    if not line.startswith("$TGY61,"):
        raise ValueError("not a $TGY61 frame")
    if "*" not in line:
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P50 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
...
def main() -> int:
    ...
    parser.add_argument("--log", default="uart_p49.txt",
                        help="decoded UART log file (default: uart_p49.txt)")
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P50 UART monitor | port={selected_port} "
        ...
        for line in lines:
            ...
            if not clean.startswith("$TGY61,"):
                # Detect the immediately previous P49/V60 firmware instead of appearing frozen.
                if clean.startswith("$TGY60,"):
                    p48_seen += 1
                    if p48_seen == 1 or (p48_seen % 10) == 0:
                        print(
                            f"UYARI: COM porttan $TGY59 geliyor (adet={p48_seen}). "
                            "Bu P49/V60 firmware demektir; P50 flash edilmemis olabilir.",
                            flush=True,
                        )
                    if log_handle is not None:
                        log_handle.write("# P49_FRAME_DETECTED " + clean + "\n")
                    continue

                if clean.startswith("$TGY59,") or clean.startswith("$TGY58,") or clean.startswith("$TGY57,") or clean.startswith("$TGY56,"):
                    older_seen += 1
                    if older_seen == 1:
                        print(
                            f"UYARI: daha eski UART frame goruldu: {clean[:6]}. "
                            "P50/V60 flash durumunu kontrol et.",
                            flush=True,
                        )
                    continue
