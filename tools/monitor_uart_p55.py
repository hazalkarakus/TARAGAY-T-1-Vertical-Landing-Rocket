#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P55 $TGY66 UART telemetry (303 fields)."""
...
def decode_frame(line: str) -> dict[str, str]:
    line = line.strip()
    if not line.startswith("$TGY66,"):
        raise ValueError("not a $TGY66 frame")
    if "*" not in line:
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P55 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
...
def main() -> int:
    ...
    parser.add_argument("--log", default="uart_p53.txt",
                        help="decoded UART log file (default: uart_p53.txt)")
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P55 UART monitor | port={selected_port} "
        ...
        for line in lines:
            ...
            if not clean.startswith("$TGY66,"):
                # Detect recent pre-P55 firmware instead of appearing frozen.
                if clean.startswith("$TGY65,") or clean.startswith("$TGY64,") or clean.startswith("$TGY63,") or clean.startswith("$TGY62,"):
                    previous_seen += 1
                    if previous_seen == 1 or (previous_seen % 10) == 0:
                        print(
                            f"UYARI: COM porttan eski TGY frame geliyor (adet={previous_seen}, prefix={clean[:6]}). "
                            "P54/P53/P52/P51 firmware olabilir; P55 flash edilmemis olabilir.",
                            flush=True,
                        )
                ...
                if clean.startswith("$TGY61,") or clean.startswith("$TGY60,") or clean.startswith("$TGY59,") or clean.startswith("$TGY58,") or clean.startswith("$TGY57,") or clean.startswith("$TGY56,"):
                    ...
                    if older_seen == 1:
                        print(
                            ...
                            "P55/V66 flash durumunu kontrol et.",
                            flush=True,
                        )
                    continue
