#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P49 $TGY60 UART telemetry (258 fields)."""
...
def decode_frame(line: str) -> dict[str, str]:
    line = line.strip()
    if not line.startswith("$TGY60,"):
        raise ValueError("not a $TGY60 frame")
    if "*" not in line:
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P49 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
...
def main() -> int:
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P49 UART monitor | port={selected_port} "
        ...
        for line in lines:
            ...
            if not clean.startswith("$TGY60,"):
                # Detect the immediately previous P48/V59 firmware instead of appearing frozen.
                if clean.startswith("$TGY59,"):
                    p48_seen += 1
                    if p48_seen == 1 or (p48_seen % 10) == 0:
                        print(
                            f"UYARI: COM porttan $TGY59 geliyor (adet={p48_seen}). "
                            "Bu P48/V59 firmware demektir; P49 flash edilmemis olabilir.",
                            flush=True,
                        )
                    if log_handle is not None:
                        log_handle.write("# P48_FRAME_DETECTED " + clean + "\n")
                    continue

                if clean.startswith("$TGY58,") or clean.startswith("$TGY57,") or clean.startswith("$TGY56,"):
                    older_seen += 1
                    if older_seen == 1:
                        print(
                            f"UYARI: daha eski UART frame goruldu: {clean[:6]}. "
                            "P49/V60 flash durumunu kontrol et.",
                            flush=True,
                        )
                    continue
