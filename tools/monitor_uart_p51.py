#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P51 $TGY62 UART telemetry (258 fields)."""
...
def decode_frame(line: str) -> dict[str, str]:
    line = line.strip()
    if not line.startswith("$TGY62,"):
        raise ValueError("not a $TGY62 frame")
    if "*" not in line:
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P51 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
...
def main() -> int:
    ...
    parser.add_argument("--log", default="uart_p51.txt",
                        help="decoded UART log file (default: uart_p51.txt)")
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P51 UART monitor | port={selected_port} "
        ...
        for line in lines:
            ...
            if not clean.startswith("$TGY62,"):
                # Detect the immediately previous P50/V61 firmware instead of appearing frozen.
                if clean.startswith("$TGY61,"):
                    p50_seen += 1
                    if p50_seen == 1 or (p50_seen % 10) == 0:
                        print(
                            f"UYARI: COM porttan $TGY61 geliyor (adet={p50_seen}). "
                            "Bu P50/V61 firmware demektir; P51 flash edilmemis olabilir.",
                            flush=True,
                        )
                    if log_handle is not None:
                        log_handle.write("# P50_FRAME_DETECTED " + clean + "\n")
                    continue

                if clean.startswith("$TGY60,") or clean.startswith("$TGY59,") or clean.startswith("$TGY58,") or clean.startswith("$TGY57,") or clean.startswith("$TGY56,"):
                    older_seen += 1
                    if older_seen == 1:
                        print(
                            f"UYARI: daha eski UART frame goruldu: {clean[:6]}. "
                            "P51/V62 flash durumunu kontrol et.",
                            flush=True,
                        )
                    continue
