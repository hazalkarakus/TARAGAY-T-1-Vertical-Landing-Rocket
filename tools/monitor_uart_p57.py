#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P57 $TGY68 UART telemetry (329 fields)."""
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P57 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
...
def compact_status(f: dict[str, str]) -> str:
    return (
        ...
        f"Baro={int(f['baro_alt_mm'])/1000.0:+.3f}m P/G={f['baro_pressure_pa']}/{f['baro_ground_pa']} "
        f"Bref={f['baro_ref_track_active']}/{f['baro_ref_frozen']}/{f['baro_ref_updates']} "
        ...
        f"fastCfg={f['imu_fast_cfg_success']}/{f['imu_fast_cfg_attempts']} "
        f"cfgFail={f['imu_fast_cfg_failures']} cfgUs={f['imu_fast_cfg_last_us']}/{f['imu_fast_cfg_max_us']} "
        f"staleFast={f['imu_stale_retry_success']}/{f['imu_stale_fast_checks']} "
        f"staleCfg={f['imu_stale_fast_success']}/{f['imu_stale_fast_attempts']} "
        f"staleFail/Esc={f['imu_stale_fast_failures']}/{f['imu_stale_recovery_escalations']} "
        f"staleUs={f['imu_stale_fast_last_us']}/{f['imu_stale_fast_max_us']} "
        f"warm={f['imu_stale_warmup_success']}/{f['imu_stale_warmup_events']} "
        f"to={f['imu_stale_warmup_timeouts']} polls={f['imu_stale_warmup_polls']} "
        f"readyUs={f['imu_stale_warmup_first_ready_us']}/{f['imu_stale_warmup_max_ready_us']} "
        f"status={f['imu_stale_warmup_last_status']} active={f['imu_stale_warmup_active']} "
        f"staleEvt={f['imu_stale_diag_event_count']}@{f['imu_stale_diag_time_ms']}ms age={f['imu_stale_diag_age_us']}us "
        f"staleRegs={f['imu_stale_diag_reg_valid']}:{f['imu_stale_diag_whoami']}/{f['imu_stale_diag_ctrl1_xl']}/"
        f"{f['imu_stale_diag_ctrl2_g']}/{f['imu_stale_diag_ctrl3_c']}/{f['imu_stale_diag_ctrl4_c']} "
        f"patEvt={f['imu_pat_event_count']}@{f['imu_pat_time_ms']}ms mag={f['imu_pat_mag']} "
        f"regs={f['imu_pat_reg_valid']}:{f['imu_pat_whoami']}/{f['imu_pat_ctrl1_xl']}/"
        f"{f['imu_pat_ctrl2_g']}/{f['imu_pat_ctrl3_c']}/{f['imu_pat_ctrl4_c']} "
        ...
        f"Gsp={f['grav_joseph_updates']}/{f['grav_joseph_faults']} "
        f"Zsp={f['zupt_joseph_updates']}/{f['zupt_joseph_faults']} "
        ...
        f"fifo={f['sd_fifo_order_faults']}/{f['sd_fifo_last_started_order']}/{f['sd_fifo_next_ready_order']} "
...
def main() -> int:
    ...
    parser.add_argument("--log", default="uart_p56.txt",
                        help="decoded UART log file (default: uart_p56.txt)")
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P57 UART monitor | port={selected_port} "
                ...
                if clean.startswith("$TGY67,") or clean.startswith("$TGY66,") or clean.startswith("$TGY65,") or clean.startswith("$TGY64,") or clean.startswith("$TGY63,") or clean.startswith("$TGY62,"):
                    ...
                    if previous_seen == 1 or (previous_seen % 10) == 0:
                        print(
                            ...
                            "P56/P55/P54/P53/P52/P51 firmware olabilir; P57 flash edilmemis olabilir.",
                            flush=True,
                        )
                ...
                if clean.startswith("$TGY61,") or clean.startswith("$TGY60,") or clean.startswith("$TGY59,") or clean.startswith("$TGY58,") or clean.startswith("$TGY57,") or clean.startswith("$TGY56,"):
                    ...
                    if older_seen == 1:
                        print(
                            ...
                            "P57/V68 flash durumunu kontrol et.",
                            flush=True,
                        )
                    continue
