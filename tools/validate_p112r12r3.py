...
checks=[
('version',has('App/Common/app_version.h','8.19M-P112R12R3-PRODUCTION-AUTONOMOUS-950PLUS-INERT-CLEAN')),
('project name',has('.project','<name>P112R12R3_PROD_AUTO_950PLUS_INERT_CLEAN</name>')),
('R12 production profile retained',has('App/Common/app_config.h','#define APP_P112R12_PRODUCTION_AUTONOMOUS_950PLUS_REV          1U')),
('R12R1 same-target guard retained',has('App/Common/app_config.h','#define APP_P112R12R1_SAME_TARGET_GUARD_REV                     1U')),
('R12R3 clean profile',has('App/Common/app_config.h','#define APP_P112R12R3_CLEAN_PRODUCTION_CHAIN_REV                 1U')),
...
('R12R2 target-step removed',lacks('App/Common/app_config.h','APP_P112R12R2_TARGET_STEP_TEST_MODE')),
('R12R2R1 max-hold removed',lacks('App/Common/app_config.h','APP_P112R12R2R1_BREAKAWAY_MAX_HOLD_REV')),
('R12R2R2 full-kick removed',lacks('App/Common/app_config.h','APP_P112R12R2R2_BREAKAWAY_FULL_KICK_REV')),
('no 255 kick symbol in P110',lacks('App/Services/NeedleValveIntegrationTest/needle_valve_integration_test.c','P110_BREAKAWAY_KICK_PWM')),
...
('do not fly marker',(R/'P112R12R3_DO_NOT_FLY.txt').is_file()),
]
# Behavior-bearing files must be byte-identical to the clean R12R1 production profile.
expected={
'App/Services/NeedleValveAutonomousControl/needle_valve_autonomous_control.c':'759ff1eaffe925c43a569ba64e964d4aa3acf4ab5f8dcb6d49f06afd6966b877',
...
for rel,want in expected.items(): checks.append((f'clean R12R1 behavior unchanged: {rel}',sha(rel)==want))
...
if shutil.which('gcc'):
    cmd=['gcc','-std=c11','-Wall','-Wextra','-Werror','-Itools/p112r12_host_stub','-IApp',
         ...
         '-o','/tmp/p112r12r3_auto']
    subprocess.run(cmd,cwd=R,check=True); subprocess.run(['/tmp/p112r12r3_auto'],cwd=R,check=True)
    cmd=['gcc','-std=c11','-Wall','-Wextra','-Werror','-Itools/p112r12_host_stub','-IApp',
         ...
         '-o','/tmp/p112r12r3_guard']
    subprocess.run(cmd,cwd=R,check=True); subprocess.run(['/tmp/p112r12r3_guard'],cwd=R,check=True)
else:
    print('SKIP: host gcc unavailable')
print('P112R12R3 clean production-autonomous inert validation: PASS')
