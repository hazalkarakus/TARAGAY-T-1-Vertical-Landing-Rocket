...
int main(void)
{
    ...
    assert(NeedleValveAutonomousControl_IsReady() == 0U);

    p83_feedback_valid = 1U;
    p83_mode = 1U;
    p83_filtered_adc = 1018U;
    ...
    assert(NeedleValveAutonomousControl_IsReady() == 1U);

    p112r5_host_tick_ms = 10U;
    assert(NeedleValveAutonomousControl_SubmitCommand(1.0f) == 1U);
    NeedleValveAutonomousControl_TimerTickISR();
    assert(request_count == 1U);
    assert(requested_target == 238U);

    p112r5_host_tick_ms = 11U;
    assert(NeedleValveAutonomousControl_SubmitCommand(0.5f) == 1U);
    NeedleValveAutonomousControl_TimerTickISR();
    assert(retarget_count == 1U);
    assert(requested_target == 628U);

    p83_filtered_adc = 630U;
    p110_final_error_adc = 2;
    ...
    p112r5_host_tick_ms = 20U;
    assert(NeedleValveAutonomousControl_SubmitCommand(0.0f) == 1U);
    NeedleValveAutonomousControl_TimerTickISR();
    assert(request_count == 2U);
    assert(requested_target == 1018U);

    NeedleValveAutonomousControl_RevokeAuthorization();
    status = NeedleValveAutonomousControl_GetStatus();
    assert(status.command_authorized == 0U);
    assert(force_safe_count >= 1U);

    p110_state = NEEDLE_ADAPTIVE_STATE_DONE;
    p110_result = NEEDLE_ADAPTIVE_RESULT_PASS;
    p112r5_host_tick_ms = 30U;
    assert(NeedleValveAutonomousControl_SubmitCommand(0.5f) == 1U);
    p112r5_host_tick_ms = 81U;
    NeedleValveAutonomousControl_TimerTickISR();
    assert(NeedleValveAutonomousControl_HasFault() == 1U);
    status = NeedleValveAutonomousControl_GetStatus();
    assert(status.fault == NEEDLE_AUTONOMOUS_FAULT_COMMAND_TIMEOUT);

    puts("P112R5 autonomous supervisor host test: PASS");
    return 0;
}
