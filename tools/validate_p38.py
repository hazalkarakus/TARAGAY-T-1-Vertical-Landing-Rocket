...
req('App/Common/app_version.h','8.19M-P38-LIDAR-WATCHDOG-REARM')
...
# P38 regression guards.
assert 'lidar_watchdog_armed' in lidar
assert 'lidar_recovery_confirmation_pending' in lidar
assert 'Lidar_BeginFirstSampleGrace' in lidar
assert 'LIDAR_FIRST_SAMPLE_GRACE_MS' in lidar
assert 'lidar_recovery_success_count++;' in lidar
# Recovery restart must not count success merely because Mem_Write_DMA started.
restart=lidar.split('case LIDAR_RECOVERY_STEP_RESTART:',1)[1].split('default:',1)[0]
assert 'lidar_recovery_success_count++' not in restart
# Connected retry path must not call StartMeasurementDMA after TryConnect returns.
conn=lidar.split('if (lidar_data.connected == 0U)',1)[1].split('if (lidar_data.state == LIDAR_STATE_ERROR)',1)[0]
assert conn.count('Lidar_StartMeasurementDMA') == 0
assert '(void)Lidar_TryConnect();' in conn
assert '(lidar_watchdog_armed != 0U)' in lidar
print('P38 validation: PASS (LiDAR watchdog grace/re-arm + real-sample recovery confirmation + no double DMA start + 206 UART fields)')
