#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P54 $TGY65 UART telemetry (297 fields)."""
...
def decode_frame(line: str) -> dict[str, str]:
    line = line.strip()
    if not line.startswith("$TGY65,"):
        raise ValueError("not a $TGY65 frame")
    if "*" not in line:
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P54 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
...
def compact_status(f: dict[str, str]) -> str:
    return (
        ...
        f"regs={f['imu_pat_reg_valid']}/{f['imu_pat_whoami']}/{f['imu_pat_ctrl1_xl']}/{f['imu_pat_ctrl2_g']}/{f['imu_pat_ctrl3_c']}/{f['imu_pat_ctrl4_c']} "
...
def main() -> int:
    ...
    parser.add_argument("--log", default="uart_p54.txt",
                        help="decoded UART log file (default: uart_p54.txt)")
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P54 UART monitor | port={selected_port} "
        ...
        for line in lines:
            ...
            if not clean.startswith("$TGY65,"):
                # Detect the immediately previous P51/V62 firmware instead of appearing frozen.
                if clean.startswith("$TGY64,") or clean.startswith("$TGY63,") or clean.startswith("$TGY62,"):
                    previous_seen += 1
                    if previous_seen == 1 or (previous_seen % 10) == 0:
                        print(
                            f"UYARI: COM porttan $TGY64/$TGY63/$TGY62 geliyor (adet={previous_seen}). "
                            "Bu daha eski firmware demektir; P54 flash edilmemis olabilir.",
                            flush=True,
                        )
                ...
                if clean.startswith("$TGY61,") or clean.startswith("$TGY60,") or clean.startswith("$TGY59,") or clean.startswith("$TGY58,") or clean.startswith("$TGY57,") or clean.startswith("$TGY56,"):
                    ...
                    if older_seen == 1:
                        print(
                            ...
                            "P54/V65 flash durumunu kontrol et.",
                            flush=True,
                        )
                    continue
