#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P48 $TGY59 UART telemetry (253 fields)."""
...
def decode_frame(line: str) -> dict[str, str]:
    line = line.strip()
    if not line.startswith("$TGY59,"):
        raise ValueError("not a $TGY59 frame")
    if "*" not in line:
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P48 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
...
def main() -> int:
    ...
    parser.add_argument("--log", default="uart_p48.txt",
                        help="decoded UART log file (default: uart_p48.txt)")
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P48 UART monitor | port={selected_port} "
        ...
        for line in lines:
            ...
            if not clean.startswith("$TGY59,"):
                # Important: detect the immediately previous P47/V58 firmware instead of appearing frozen.
                if clean.startswith("$TGY58,"):
                    p47_seen += 1
                    if p47_seen == 1 or (p47_seen % 10) == 0:
                        print(
                            f"UYARI: COM porttan $TGY58 geliyor (adet={p47_seen}). "
                            "Bu P47/V58 firmware demektir; P48 flash edilmemis olabilir.",
                            flush=True,
                        )
                    if log_handle is not None:
                        log_handle.write("# P47_FRAME_DETECTED " + clean + "\n")
                    continue
