#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P60 UART telemetry in VS Code.

P60 keeps the P57 $TGY68 / 329-field UART frame. The default screen view
emphasizes NRF, tahliye/RCS, E-STOP, needle actuator, IMU/ESKF, SD and CPU.
"""
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P60 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
...
def compact_status(f: dict[str, str]) -> str:
    ...
    return (
        ...
        f"RCS req/applied={f['rcs_req_mask']}/{f['rcs_applied_mask']} "
...
def main() -> int:
    ...
    parser.add_argument("--log", default="uart_p59.txt",
                        help="decoded UART log file (default: uart_p59.txt)")
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P60 UART monitor | port={selected_port} "
                ...
                if clean.startswith("$TGY61,") or clean.startswith("$TGY60,") or clean.startswith("$TGY59,") or clean.startswith("$TGY58,") or clean.startswith("$TGY57,") or clean.startswith("$TGY56,"):
                    ...
                    if older_seen == 1:
                        print(
                            ...
                            "P60/V68 flash durumunu kontrol et.",
                            flush=True,
                        )
                    continue
