#!/usr/bin/env python3
"""TARAGAY-T1 R8R35R3R10R3 final-candidate inert monitor."""
...
def main():
    ...
    ap.add_argument('--out',default='uart_p112r12r8r35r3r10r3_final_candidate.txt')
    a=ap.parse_args()
    import serial
    rows=[]; rejected=0; last_fail_count=0
    t0=time.monotonic(); out=Path(a.out)
    print('R8R35R3R10R3 FINAL CANDIDATE - FLIGHT RX-ONLY NRF + HOLD-SAFE NEEDLE + RAM-FIRST SD')
    print('BASINCISIZ/INERT: READY=1 bekle, PE9 ayir; needle motor gucu BAGLI olsun.')
    print('PE9 sonrasi: SDIO yeni komut YOK, nRF rocket downlink TX YOK; radio command-priority PRX. 45-60 s calistir, sonra E-STOP ver. STOP rokete ulasmali, needle CLOSED ref civarina donmeli, RAM->SD replay tamamlanmali.')
    ...
    with serial.Serial(a.port,baudrate=a.baud,timeout=.25) as ser, out.open('w',encoding='utf-8') as fp:
        ...
        while a.duration<=0 or time.monotonic()-t0<a.duration:
            ...
            print(
                ...
                f"NRF={iv(f,'nrf_link')} age={iv(f,'nrf_age_ms'):4d} tlmTX={iv(f,'nrf_tlm_tx_start')} STOP={iv(f,'stop_latched')} EClose={iv(f,'estop_close_active')}/{iv(f,'estop_close_complete')}/{iv(f,'estop_close_failed')} "
    ...
    print('\n================ R8R35R3R10R3 FINAL CANDIDATE OZET =================')
    ...
    print(f"NEW nRF telemetry TX starts in flight: {nrf_tlm_flight_delta} (must be 0)")
    print(f"Valid command packets gained in flight: {nrf_rx_flight_delta}")
    print(f"nRF link-down sampled frames      : {nrf_link_drop_samples}")
    ...
    if stop_seen:
        estop_ok = (iv(last,'stop_latched')==1 and
                    ...
                    iv(last,'sd_r10_flush_complete')==1)
    passed=(iv(last,'sd_r10_ram_mode')==1 and iv(last,'sd_r10_ram_overflow')==0 and
            iv(last,'sd_r10_physical_sd_faults')==0 and iv(last,'sd_dma_start_errors_r4')==0 and
            iv(last,'runtime_hal_delay_violations')==0 and capture_hz>=6.5 and clkdiv==10 and start_delta==0 and
            nrf_tlm_flight_delta==0 and nrf_rx_flight_delta>0 and hold_fault_samples==0 and
            estop_ok)
    print('RESULT                         : '+('PASS' if passed else 'CHECK/FAIL'))
    print('==================================================')
    return 0
