#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P53 $TGY64 UART telemetry (297 fields)."""
...
def decode_frame(line: str) -> dict[str, str]:
    line = line.strip()
    if not line.startswith("$TGY64,"):
        raise ValueError("not a $TGY64 frame")
    if "*" not in line:
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P53 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
...
def main() -> int:
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P53 UART monitor | port={selected_port} "
        ...
        for line in lines:
            ...
            if not clean.startswith("$TGY64,"):
                # Detect the immediately previous P51/V62 firmware instead of appearing frozen.
                if clean.startswith("$TGY63,") or clean.startswith("$TGY62,"):
                    previous_seen += 1
                    if previous_seen == 1 or (previous_seen % 10) == 0:
                        print(
                            f"UYARI: COM porttan $TGY63/$TGY62 geliyor (adet={previous_seen}). "
                            "Bu P52/V63 veya P51/V62 firmware demektir; P53 flash edilmemis olabilir.",
                            flush=True,
                        )
                ...
                if clean.startswith("$TGY61,") or clean.startswith("$TGY60,") or clean.startswith("$TGY59,") or clean.startswith("$TGY58,") or clean.startswith("$TGY57,") or clean.startswith("$TGY56,"):
                    ...
                    if older_seen == 1:
                        print(
                            ...
                            "P53/V64 flash durumunu kontrol et.",
                            flush=True,
                        )
                    continue
