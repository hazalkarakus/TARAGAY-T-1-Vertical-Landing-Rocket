#!/usr/bin/env python3
"""TARAGAY-T1 P112R12R8R27 three-pose IMU->rocket frame calibration + relay bench observer."""
from __future__ import annotations
import argparse, binascii, json, statistics, sys, time
from pathlib import Path

PREFIX="$TGY73,"; N=220; BAUD=115200
FIELDS=(
...
CAL_PHASE={0:"WAIT_SOURCE",1:"HOLD_UPRIGHT",2:"TILT_TOP_TO_PLUS_X",3:"HOLD_PLUS_X_TILT",4:"TILT_TOP_TO_PLUS_Y",5:"HOLD_PLUS_Y_TILT",6:"CALIBRATED",7:"CAL_FAULT"}
...
def serial_lines(port,baud,duration):
    ...
    with serial.Serial(port,baudrate=baud,timeout=.5) as s:
        ...
        print("="*110)
        print("R8R27 THREE-POSE IMU -> ROCKET FRAME CALIBRATION + PHYSICAL RELAY BENCH")
        print("BASINCSIZ. NEEDLE/MOTOR CALISMAMALI. PE9 GEREKMEZ.")
        print("1) READY sonrasi roketi TAM DIK ve SABIT tut.")
        print("2) TILT_TOP_TO_PLUS_X gorunce roketin USTUNU fiziksel/CAD +X yonune 12-20 deg eg ve sabit tut.")
        print("3) TILT_TOP_TO_PLUS_Y gorunce dik konuma don, sonra USTU fiziksel/CAD +Y yonune 12-20 deg eg.")
        print("4) CALIBRATED gorunce dik konuma don; sonra +/-X ve +/-Y egip roleleri gozle.")
        print("="*110)
...
def status(f):
    ...
    return (f"t={iv(f,'time_ms')/1000:7.2f}s READY={iv(f,'ready')} SYS={iv(f,'system_ok')}/{iv(f,'system_fault')} "
            ...
            f"upr/X/Y={iv(f,'cal_upright_samples')}/{iv(f,'cal_tilt_samples')}/{iv(f,'cal_y_tilt_samples')} "
            f"tiltX/Y={iv(f,'cal_tilt_cdeg')/100:.1f}/{iv(f,'cal_y_tilt_cdeg')/100:.1f}deg "
            f"XY={iv(f,'cal_xy_angle_cdeg')/100:.1f}deg agree={iv(f,'cal_axis_agree_x10000')/10000:.3f} | "
...
def run(a):
    ...
    try:
        for line in lines:
            ...
            if ph!=last_phase:
                ...
                if ph==1: print(">>> ROKETI DIK VE SABIT TUT. Yaklasik 2 saniye.")
                elif ph==2: print(">>> SIMDI ROKETIN USTUNU FIZIKSEL/CAD +X YONUNE 12-20 DEG EG.")
                elif ph==3: print(">>> BU +X EGIMINI SABIT TUT. Yaklasik 1.2 saniye.")
                elif ph==4: print(">>> +X KAYDEDILDI. DIK KONUMA DON; SIMDI USTU FIZIKSEL/CAD +Y YONUNE 12-20 DEG EG.")
                elif ph==5: print(">>> BU +Y EGIMINI SABIT TUT. Yaklasik 1.2 saniye.")
                elif ph==6:
                    print(">>> 3-POZ KALIBRASYON TAMAM. DIK KONUMA DON VE +/-X +/-Y ROLE TESTINI YAP.")
                    r=matrix(f);print(">>> R_rocket_from_imu =");[print("    "," ".join(f"{x:+.4f}" for x in row)) for row in r]
                    print(f">>> QUALITY XY={iv(f,'cal_xy_angle_cdeg')/100:.2f}deg agree={iv(f,'cal_axis_agree_x10000')/10000:.4f} det={iv(f,'cal_det_x10000')/10000:.4f} ortho={iv(f,'cal_ortho_err_x10000')/10000:.5f}")
                elif ph==7: print(">>> KALIBRASYON HATASI. Gucu kesmeden logu kaydet ve incele.")
                last_phase=ph
    ...
    print(f"Calibration tilt X/Y       : {iv(latest,'cal_tilt_cdeg')/100:.2f} / {iv(latest,'cal_y_tilt_cdeg')/100:.2f} deg")
    ...
    quality_ok=(7500 <= iv(latest,"cal_xy_angle_cdeg") <= 10500 and
                iv(latest,"cal_axis_agree_x10000") >= 9000 and
    ...
    print("\nSONUC:","PASS - R8R27 THREE-POSE CALIBRATION" if pass_ok else "INCELE - calibration/geometry/gate tamamlanmadi")
...
def main():
    ...
    p.add_argument("--log",default="uart_p112r12r8r27_three_pose_imu_rocket_frame_cal.txt");p.add_argument("--capture");p.add_argument("--print-every",type=int,default=3)
