...
#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P78 needle-valve breakaway-timing characterization bench firmware.

P78 keeps the $TGY68 diagnostic stream, measures time-to-first-motion at PWM75, keeps NRF disabled and uses UART only for diagnostics.
"""
...
FIELDS = (
    ...
    'p78_adc_500ms',
    'p78_delta_500_adc',
    'p78_max_excursion_adc',
    'p78_fast_adc_samples',
)
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P78 UART breakaway-timing characterization | port={port} baud={baud} fields={len(FIELDS)}")
...
def compact_status(f: dict[str, str]) -> str:
    stage_names = {0: "CAPTURE_REF", 1: "READY", 2: "TEST", 3: "DONE", 4: "ABORT"}
    result_names = {
        0: "NOT_RUN",
        1: "RUNNING",
        2: "BREAKAWAY_FOUND",
    ...
    stage = iv("p78_stage")
    result = iv("p78_result")
    phase = iv("p78_phase")
    direction = iv("p78_direction")
    adc = iv("needle_adc")
    ref = iv("p78_ref_adc")
    start_adc = iv("p78_start_adc")
    detect_adc = iv("p78_detect_adc")
    brake_adc = iv("p78_brake_adc")
    a100 = iv("p78_adc_100ms")
    a250 = iv("p78_adc_250ms")
    a500 = iv("p78_adc_500ms")
    delta = iv("p78_delta_500_adc")
    max_exc = iv("p78_max_excursion_adc")
    pwm = iv("p78_pwm")
    max_ms = iv("p78_max_pulse_ms")
    actual_ms = iv("p78_actual_pulse_ms")
    detect_ms = iv("p78_detect_ms")
    detected = iv("p78_motion_detected")
    samples = iv("p78_fast_adc_samples")
    ...
    if f.get("system_ok", "0") != "1":
        ...
        hint = "USER #2: PWM75 / max80ms breakaway timing testini baslat"
    elif phase in (1, 2):
        hint = "DOKUNMA: test/settle aktif; USER basisi ABORT eder"
    elif stage == 3 and result == 2:
        hint = "PASS: BREAKAWAY bulundu. Daha fazla butona basma; logu gonder."
    elif stage == 3 and result == 3:
        hint = "NO MOTION: 80ms doldu. Daha fazla butona basma; logu gonder."
    else:
        hint = "ABORT: driver kapali; logu gonder"
    ...
    return (
        ...
        f"P78 {stage_names.get(stage, stage)}/{phase_names.get(phase, phase)} "
        f"res={result_names.get(result, result)} btn={iv('p78_button_count')} | "
        f"dir={dir_names.get(direction, direction)} PWM={pwm} max/actual/detect={max_ms}/{actual_ms}/{detect_ms}ms "
        f"det={detected} samples={samples} | "
        f"ADC now/ref/start/detect/brake/100/250/500={adc}/{ref}/{start_adc}/{detect_adc}/{brake_adc}/{a100}/{a250}/{a500} "
...
def main() -> int:
    ...
    parser.add_argument("--log", default="uart_p78_breakaway_timing.txt",
                        help="decoded UART log file (default: uart_p78_breakaway_timing.txt)")
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P78 UART breakaway-timing characterization | port={selected_port} "
                ...
                if clean.startswith("$TGY67,") or clean.startswith("$TGY66,") or clean.startswith("$TGY65,") or clean.startswith("$TGY64,") or clean.startswith("$TGY63,") or clean.startswith("$TGY62,"):
                    ...
                    if previous_seen == 1 or (previous_seen % 10) == 0:
                        print(
                            ...
                            "Eski firmware olabilir; P78 $TGY68 frame kullanmalidir.",
                            flush=True,
                        )
