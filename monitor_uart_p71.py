#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P71 normal rocket UART telemetry + NRF TDD diagnostic counters.

P71 keeps the proven $TGY68 diagnostic stream and appends 7 read-only NRF TDD counters.
The flight/control/sensor logic is unchanged; UART remains bench diagnostics.
"""
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P71 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
...
def compact_status(f: dict[str, str]) -> str:
    """Short P60 bench view; the log file still stores every decoded field."""
    try:
        flags = int(f["nrf_flags"], 0)
    except ValueError:
        flags = 0

    tahliye = 1 if (flags & 0x01) else 0
    estop_cmd = 1 if (flags & 0x02) else 0
    ...
    return (
        f"seq={f['seq']:>5s} t={f['time_ms']:>8s}ms | "
        f"SYS={f['system_ok']}/{f['system_fault']} flight={f['flight_active']} auth={f['actuator_authorized']} | "
        f"NRF conn/link={f['nrf_connected']}/{f['nrf_link']} "
        f"flags={f['nrf_flags']} TAH={tahliye} ESTOPcmd={estop_cmd} "
        f"age={age}ms rx={f['nrf_rx_count']} err={f['nrf_errors']} inv={f['nrf_invalid']} | "
        f"TLM sch/start/ok/fail={f['nrf_tlm_schedule']}/{f['nrf_tlm_tx_start']}/{f['nrf_tlm_tx_success']}/{f['nrf_tlm_tx_fail']} "
        f"repl={f['nrf_tlm_pending_replace']} txus={f['nrf_tlm_last_tx_us']}/{f['nrf_tlm_max_tx_us']} | "
        f"STOP_LATCH={f['stop_latched']} | "
        f"RCS req/applied={f['rcs_req_mask']}/{f['rcs_applied_mask']} pair={vent_pair_label(f['rcs_applied_mask'])} "
        f"state/fault={f['rcs_state']}/{f['rcs_fault']} | "
        f"NEEDLE en/lock/fault={f['needle_enabled']}/{f['needle_lock']}/{f['needle_fault']} "
        f"ADC={f['needle_adc']} tgt={f['needle_target_adc']} PWM={f['needle_rpwm']}/{f['needle_lpwm']} | "
        f"IMU={f['imu_valid']} rec={f['imu_recovery_count']}/{f['imu_recovery_failures']} "
        f"ESKF={f['eskf_ok']} div={f['eskf_div_count']} | "
        f"SD={f['sd_ready']}/{f['sd_logging']} err/drop={f['sd_errors']}/{f['sd_dropped']} "
        f"frames={f['sd_frames']} | CPU={int(f['cpu_x100'])/100.0:.1f}% "
        f"missNRF={f['miss_nrf']}"
    )
...
def main() -> int:
    ...
    parser.add_argument("--log", default="uart_p71.txt",
                        help="decoded UART log file (default: uart_p71.txt)")
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P71 UART monitor | port={selected_port} "
                ...
                if clean.startswith("$TGY67,") or clean.startswith("$TGY66,") or clean.startswith("$TGY65,") or clean.startswith("$TGY64,") or clean.startswith("$TGY63,") or clean.startswith("$TGY62,"):
                    ...
                    if previous_seen == 1 or (previous_seen % 10) == 0:
                        print(
                            ...
                            "Eski firmware olabilir; P60 tabani $TGY68 frame kullanmalidir.",
                            flush=True,
                        )
                ...
                if clean.startswith("$TGY61,") or clean.startswith("$TGY60,") or clean.startswith("$TGY59,") or clean.startswith("$TGY58,") or clean.startswith("$TGY57,") or clean.startswith("$TGY56,"):
                    ...
                    if older_seen == 1:
                        print(
                            ...
                            "P67/V68 flash durumunu kontrol et.",
                            flush=True,
                        )
                    continue
