...
def main():
    ...
    ap.add_argument('--out',default='uart_p112r12r8r35r3r9_sdio_750khz_actuator_quiet.txt')
    a=ap.parse_args()
    import serial
    rows=[]; rejected=0; last_fail_count=0
    t0=time.monotonic(); out=Path(a.out)
    print('R8R35R3R9 SDIO 750KHZ + ACTUATOR-QUIET 30HZ - LIVE')
    print('BASINCISIZ/INERT: READY=1 bekle, PE9 ayir; needle motor gucu BAGLI olsun.')
    print('30 Hz RAM capture + monotonic flight/motor/RCS write hold + real BSP SDIO ClockDiv telemetry aktif.')
    ...
    with serial.Serial(a.port,baudrate=a.baud,timeout=.25) as ser, out.open('w',encoding='utf-8') as fp:
        ...
        while a.duration<=0 or time.monotonic()-t0<a.duration:
            ...
            print(
                ...
                f"hold={iv(f,'sd_hold_active_r9')}/{iv(f,'sd_hold_remaining_ms_r9')}ms "
                f"rcsev={iv(f,'sd_rcs_hold_events_r9')} div={iv(f,'sd_sdio_clkcr_r9') & 0xFF} "
                f"suppress={iv(f,'sd_suppressed_total')}"
            )
    ...
    print(f"RAM capture/push/pop [Hz]     : {capture_hz:.2f}/{push_hz:.2f}/{pop_hz:.2f}")
    print(f"Ring final/high-water         : {iv(last,'sd_ring_count')}/{iv(last,'sd_ring_high_water')}")
    clkcr=iv(last,'sd_sdio_clkcr_r9'); clkdiv=clkcr & 0xFF
    sdio_hz=48000000.0/(clkdiv+2) if clkdiv<=255 else 0.0
    print(f"Actuator write suppressions   : {iv(last,'sd_suppressed_total')}")
    print(f"Hold final / remaining ms     : {iv(last,'sd_hold_active_r9')}/{iv(last,'sd_hold_remaining_ms_r9')}")
    print(f"RCS hold events               : {iv(last,'sd_rcs_hold_events_r9')}")
    ...
    passed=(iv(last,'sd_ready')==1 and iv(last,'sd_logging')==1 and
            iv(last,'sd_write_errors')==0 and iv(last,'sd_dropped')==0 and
            iv(last,'sd_ring_overruns')==0 and iv(last,'sd_r7_fail_count')==0 and
            iv(last,'runtime_hal_delay_violations')==0 and capture_hz>=27.0 and clkdiv==62)
