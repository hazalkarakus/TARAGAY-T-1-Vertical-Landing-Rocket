...
def main():
    ...
    ap.add_argument('--out',default='uart_p112r12r8r35r3r6_sd_transient_start_retry.txt')
    a=ap.parse_args()
    import serial
    rows=[]; rejected=0; first_bad=None
    t0=time.monotonic()
    out=Path(a.out)
    print('R8R35R3R6 SD TRANSIENT START RETRY - LIVE')
    print('READY=1 bekle, PE9 ayir ve needle motor gucu BAGLI olsun. Start glitch retry dogrulanacak.')
    ...
    with serial.Serial(a.port,baudrate=a.baud,timeout=.25) as ser, out.open('w',encoding='utf-8') as fp:
        ...
        while a.duration<=0 or time.monotonic()-t0<a.duration:
            ...
            bad=(iv(f,'sd_ready')==0 or iv(f,'sd_logging')==0 or iv(f,'sd_write_errors')>0 or iv(f,'sd_async_timeouts')>0)
            if bad and first_bad is None: first_bad=f.copy()
            print(
                f"t={iv(f,'time_ms'):6d} ready={iv(f,'ready')} flight={iv(f,'flight_active')} "
                f"SD={iv(f,'sd_ready')}/{iv(f,'sd_logging')} ring={iv(f,'sd_ring_count'):3d} "
                f"drop={iv(f,'sd_dropped'):4d} push/pop={iv(f,'sd_ring_push_total')}/{iv(f,'sd_ring_pop_total')} "
                f"dma={iv(f,'sd_dma_start_total')}/{iv(f,'sd_dma_complete_total')} "
                f"hostdef={iv(f,'sd_host_busy_defers_r4')} halstate={iv(f,'sd_bsp_hal_state_r4')} "
                f"starterr={iv(f,'sd_dma_start_errors_r4')} wrerr={iv(f,'sd_write_errors')}"
            )
    ...
    ready=any(iv(x,'ready') for x in rows); flight=any(iv(x,'flight_active') for x in rows)
    max_ring=max(iv(x,'sd_ring_count') for x in rows)
    min_needle=min(iv(x,'needle_adc') for x in rows)
    max_bp=max(iv(x,'sd_backpressure_level') for x in rows)
    frame_rate=d('sd_frames_total')/dur; push_rate=d('sd_ring_push_total')/dur; pop_rate=d('sd_ring_pop_total')/dur
    ...
    needle_moved=(min_needle < 900)
    sd_ok=(ready and flight and needle_moved and iv(last,'sd_ready')==1 and iv(last,'sd_logging')==1 and
           ...
           max_ring<127 and frame_rate>=190.0 and push_rate>=190.0 and pop_rate>=190.0 and
           update_rate>=200.0 and d('sd_dma_complete_total')>0 and misses==0 and realign==0)
    print('\n================ R8R35R3R6 SD OZET ================')
    print(f'Duration                       : {dur:.3f}s')
    if cpu: print(f'CPU avg/max                    : {statistics.mean(cpu):.2f}% / {max(cpu):.2f}%')
    print(f'Ready/flight                   : {ready}/{flight}')
    print(f'Needle moved / min ADC         : {needle_moved}/{min_needle}')
    ...
    print(f'SD async timeout d             : {d("sd_async_timeouts")}')
    print(f'SD frame push/pop rate         : {frame_rate:.1f}/{push_rate:.1f}/{pop_rate:.1f} Hz')
    print(f'SD update rate                 : {update_rate:.1f} Hz')
    print(f'SD DMA success start/done d    : {d("sd_dma_start_total")}/{d("sd_dma_complete_total")}')
    print(f'R4 host-busy defers d          : {d("sd_host_busy_defers_r4")}')
    print(f'R6 observed DMA start glitches  : {d("sd_dma_start_errors_r4")} (0 da olabilir; >0 iken SD yasarsa retry kaniti)')
    print(f'R4 final BSP HAL state         : {iv(last,"sd_bsp_hal_state_r4")} (1=READY)')
    print(f'SD ring max/backpressure max   : {max_ring}/127 / {max_bp}')
    print(f'Scheduler misses/realign       : {misses}/{realign}')
    if first_bad is not None:
        print(f'FIRST SD BAD                   : t={iv(first_bad,"time_ms")}ms last={iv(first_bad,"sd_last_result")} wr={iv(first_bad,"sd_write_errors")} hostdef={iv(first_bad,"sd_host_busy_defers_r4")} starterr={iv(first_bad,"sd_dma_start_errors_r4")} halstate={iv(first_bad,"sd_bsp_hal_state_r4")}')
    print('\nSONUC:', 'PASS - SD survives powered-needle test' if sd_ok else 'INCELE - SD transient retry kosullari tamamlanmadi')
