#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P41 $TGY55 UART telemetry (214 fields)."""
...
FIELDS = (
    ...
    'imu_pattern_errors', 'imu_pattern_retries', 'imu_pattern_retry_success', 'imu_invalid_samples', 'imu_redundant_rejects', 'baro_valid',
...
def serial_lines(port: str, baud: int) -> Iterable[str]:
    ...
    with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
        print(f"# P41 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
        while True:
