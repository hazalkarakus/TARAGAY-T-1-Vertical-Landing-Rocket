#!/usr/bin/env python3
"""TARAGAY-T1 P112R12R8R24 real-ESKF axis/sign observer — READ ONLY."""
from __future__ import annotations
import argparse, binascii, json, statistics, sys, time
from pathlib import Path

PREFIX="$TGY73,"; N=199; BAUD=115200
FIELDS=(
...
def serial_lines(port,baud,duration):
    ...
    with serial.Serial(port,baudrate=baud,timeout=.5) as s:
        ...
        print("="*125)
        print("P112R12R8R24 REAL ESKF ROLL/PITCH TILT -> ZERO-REF RCS COMPUTE-ONLY / READ ONLY")
        print("BASINCSIZ/INERT. Motor ve RCS fiziksel olarak CALISMAMALI. Her iki attitude eksenini + ve - yone kisa sure egip merkeze getir.")
        print("NOT: fl_hpos_valid=0 iken yatay hedef GATED; RCS hedefi 0/0 derece. Event sayaclari kisa 40 ms istekleri yakalar.")
        print("="*125)
...
def status(f,tag=""):
    ...
    return (f"t={iv(f,'time_ms')/1000:7.2f}s {tag:>5} CPU={pct(f,'cpu_x100'):5.2f}% SYS={iv(f,'system_ok')}/{iv(f,'system_fault')} "
            ...
            f"axisA/B={iv(f,'fl_pitch_cdeg')/100:.2f}/{iv(f,'fl_yaw_cdeg')/100:.2f}deg L={iv(f,'fl_valve_x10000')/10000:.4f} "
            f"RCS req/app=0x{iv(f,'fl_rcs_req_mask'):X}/0x{iv(f,'fl_rcs_applied_mask'):X} gate={iv(f,'fl_horiz_gated')} "
            f"ev={iv(f,'fl_v1_events')}/{iv(f,'fl_v3_events')}/{iv(f,'fl_v5_events')}/{iv(f,'fl_v7_events')} | PHYS pwm={iv(f,'p110_pwm')} rcs={iv(f,'rcs_mask')}")
...
def run(a):
    ...
    print("\n================ R8R24 AXIS-SIGN DIAGNOSTIC OZET ================")
    ...
    print(f"Real input spans z/axisA/axisB      : {span(z_vals):.3f} m / {span(pitch_vals):.2f} deg / {span(yaw_vals):.2f} deg")
    ...
    print(f"RCS event counts V1/V3/V5/V7: {iv(latest,'fl_v1_events')}/{iv(latest,'fl_v3_events')}/{iv(latest,'fl_v5_events')}/{iv(latest,'fl_v7_events')}")
    print(f"Horizontal target gated final: {iv(latest,'fl_horiz_gated')} (expected 1 while hpos=0)")
    ...
    print("\nSONUC:", "PASS - R8R24 REAL ESKF AXIS/SIGN COMPUTE-ONLY" if pass_ok else "INCELE - yukaridaki gate'lerden biri gecmedi")
...
def main():
    ...
    p.add_argument('--log',default='uart_p112r12r8r24_tilt_axis_fix.txt'); p.add_argument('--capture'); p.add_argument('--print-every',type=int,default=3)
