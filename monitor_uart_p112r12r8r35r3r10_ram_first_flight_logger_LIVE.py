#!/usr/bin/env python3
"""TARAGAY-T1 R8R35R3R10 RAM-first flight logger 30 Hz monitor."""
...
def main():
    ...
    ap.add_argument('--out',default='uart_p112r12r8r35r3r10_ram_first_flight_logger.txt')
    a=ap.parse_args()
    import serial
    rows=[]; rejected=0; last_fail_count=0
    t0=time.monotonic(); out=Path(a.out)
    print('R8R35R3R10 RAM-FIRST FLIGHT LOGGER 30HZ - LIVE')
    print('BASINCISIZ/INERT: READY=1 bekle, PE9 ayir; needle motor gucu BAGLI olsun.')
    print('PE9 sonrasi SDIO yeni komut YOK; 30 Hz compact RAM capture. Touchdown/E-STOP + 1 s quiet -> SD replay.')
    ...
    passed=(iv(last,'sd_r10_ram_mode')==1 and iv(last,'sd_r10_ram_overflow')==0 and
            ...
            iv(last,'runtime_hal_delay_violations')==0 and capture_hz>=27.0 and clkdiv==10 and start_delta==0)
