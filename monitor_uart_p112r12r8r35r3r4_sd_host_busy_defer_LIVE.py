#!/usr/bin/env python3
"""TARAGAY-T1 R8R35R3R4 SD host-busy defer - standalone live monitor."""
...
def main():
    ...
    ap.add_argument('--out',default='uart_p112r12r8r35r3r4_sd_host_busy_defer.txt')
    a=ap.parse_args()
    import serial
    rows=[]; rejected=0; first_bad=None
    t0=time.monotonic()
    out=Path(a.out)
    print('R8R35R3R4 SD HOST-BUSY DEFER - LIVE')
    print('READY=1 bekle, PE9 ayir. Bu monitor SADECE SD kapanisini puanlar.')
    ...
    sd_ok=(ready and flight and iv(last,'sd_ready')==1 and iv(last,'sd_logging')==1 and
           ...
           d('sd_dma_start_errors_r4')==0 and max_ring<127 and frame_rate>=190.0 and push_rate>=190.0 and pop_rate>=190.0 and
    ...
    print(f'R4 BSP/HAL DMA start errors d  : {d("sd_dma_start_errors_r4")}')
    ...
    print('\nSONUC:', 'PASS - SD host-busy launch race kapandi' if sd_ok else 'INCELE - SD closure kosullari tamamlanmadi')
