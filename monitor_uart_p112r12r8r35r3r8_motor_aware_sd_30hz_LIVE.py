#!/usr/bin/env python3
"""TARAGAY-T1 R8R35R3R8 motor-aware SD 30 Hz + no-runtime-HAL_Delay monitor."""
...
def main():
    ...
    ap.add_argument('--out',default='uart_p112r12r8r35r3r8_motor_aware_sd_30hz.txt')
    a=ap.parse_args()
    import serial
    rows=[]; rejected=0; last_fail_count=0
    t0=time.monotonic(); out=Path(a.out)
    print('R8R35R3R8 MOTOR-AWARE SD 30HZ + NO-RUNTIME-HAL-DELAY - LIVE')
    print('BASINCISIZ/INERT: READY=1 bekle, PE9 ayir; needle motor gucu BAGLI olsun.')
    print('30 Hz RAM capture + motor-aware SD write hold aktif; R3R7 HAL/SDIO snapshot korunuyor.')
    ...
    print(f"Motor-aware write suppressions: {iv(last,'sd_suppressed_total')}")
    ...
    passed=(iv(last,'sd_ready')==1 and iv(last,'sd_logging')==1 and
            ...
            iv(last,'runtime_hal_delay_violations')==0 and capture_hz>=27.0)
