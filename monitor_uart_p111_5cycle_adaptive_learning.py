#!/usr/bin/env python3
# P111_5CYCLE_ADAPTIVE_LEARNING_MONITOR
"""Monitor TARAGAY-T1 P111 five-cycle adaptive needle learning commissioning."""
    ...
    try:
        with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
            ...
            try:
                ...
                pass
            print(f"# P111 UART 5-cycle adaptive learning diagnostic | port={port} baud={baud} fields={len(FIELDS)}")
...
def compact_status(f: dict[str, str]) -> str:
    ...
    return (
        f"t={iv('time_ms')/1000.0:7.2f}s | P111 {sup_state_name} result={sup_result}:{sup_result_name} "
...
def main() -> int:
    ...
    try:
        if log_file is not None:
            ...
            header = (
                f"# P111 UART 5-cycle adaptive learning diagnostic | port={selected_port} "
                ...
                if clean.startswith("$TGY67,") or clean.startswith("$TGY66,") or clean.startswith("$TGY65,") or clean.startswith("$TGY64,") or clean.startswith("$TGY63,") or clean.startswith("$TGY62,"):
                    ...
                    if previous_seen == 1 or (previous_seen % 10) == 0:
                        print(
                            ...
                            "Eski firmware olabilir; P111 $TGY68 frame kullanmalidir.",
                            flush=True,
                        )
                ...
                if clean.startswith("$TGY61,") or clean.startswith("$TGY60,") or clean.startswith("$TGY59,") or clean.startswith("$TGY58,") or clean.startswith("$TGY57,") or clean.startswith("$TGY56,"):
                    ...
                    if older_seen == 1:
                        print(
                            ...
                            "P111 flash durumunu kontrol et.",
                            flush=True,
                        )
                    continue
