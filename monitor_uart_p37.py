#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P37 $TGY55 UART telemetry (206 fields)."""
...
def serial_lines(port: str, baud: int) -> Iterable[str]:
    ...
    with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
        print(f"# P37 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
        while True:
