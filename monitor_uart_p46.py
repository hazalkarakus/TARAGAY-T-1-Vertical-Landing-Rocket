#!/usr/bin/env python3
"""Monitor TARAGAY-T1 P46 $TGY57 UART telemetry (241 fields)."""
...
FIELDS = (
    ...
    'imu_pattern_errors', 'imu_pattern_retries', 'imu_pattern_retry_success', 'imu_pattern_recovery_escalations', 'imu_invalid_samples', 'imu_redundant_rejects', 'baro_valid',
    ...
    'eskf_public_count', 'eskf_reset_reason', 'eskf_reset_count', 'eskf_num_errors',
    'eskf_public_rejects', 'eskf_z_jump_rejects', 'eskf_vz_jump_rejects',
    'eskf_cov_ok', 'eskf_cov_checks', 'eskf_cov_faults', 'eskf_cov_reinits',
    'eskf_cov_diag_min_u1e6', 'eskf_cov_diag_max_m1e3', 'eskf_cov_sym_max_u1e6',
    'eskf_cov_fault_stage', 'eskf_cov_fault_state', 'eskf_cov_fault_other',
    'eskf_cov_fault_raw_n1e9', 'eskf_cov_fault_aux_n1e9', 'eskf_cov_fault_time_ms',
    'eskf_cov_rollback_last_good', 'eskf_cov_rollbacks', 'eskf_cov_roundoff_clamps',
    'eskf_cov_roundoff_stage', 'eskf_cov_roundoff_state', 'eskf_cov_roundoff_raw_n1e9',
    'eskf_cov_roundoff_time_ms', 'baro_fresh', 'lidar_fresh', 'vertical_source_mask',
...
def decode_frame(line: str) -> dict[str, str]:
    line = line.strip()
    if not line.startswith("$TGY57,"):
        raise ValueError("not a $TGY57 frame")
    if "*" not in line:
...
def serial_lines(port: str, baud: int) -> Iterable[str]:
    ...
    with serial.Serial(port, baudrate=baud, timeout=1.0) as connection:
        print(f"# P46 UART monitor | port={port} baud={baud} fields={len(FIELDS)}")
        while True:
