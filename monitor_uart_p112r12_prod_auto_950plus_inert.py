#!/usr/bin/env python3
"""TARAGAY-T1 P112R12 production-autonomous 950+ INERT monitor - READ ONLY."""
...
def serial_lines(port,baud,dur):
 ...
 with serial.Serial(port,baudrate=baud,timeout=.5) as s:
  ...
  print('='*86)
  print('P112R12 PRODUCTION AUTONOMOUS 950+ / INERT - READ ONLY UART')
...
def run(a):
 if a.capture: lines=file_lines(a.capture); log=None
  ...
  log.write(f'# P112R12 production autonomous 950+ inert | port={a.port} baud={a.baud}\n')
 valid=rej=0; latest=None; last=None
 ready=False; pe9open=False; auth=False; mainvalid=False; maxmoves=0; maxho=0; rcs_nonzero=False; max_target_dev=0
 try:
  for l in lines:
   ...
   if base: max_target_dev=max(max_target_dev,abs(tgt-base))
   ph=phase(f)
   if ph!=last: print('\n>>>',ph); last=ph
 ...
 if not pe9open:
  ...
  print('\nSONUC: PASS - PRODUCTION CHAIN AUTHORIZED; GNC SENTETIK HAREKET URETMEDI')
 else:
  print('\nSONUC: PASS - PRODUCTION AUTONOMOUS NEEDLE CHAIN MOTOR HAREKETI GOZLENDI')
 return 0

def main():
 p=argparse.ArgumentParser();p.add_argument('capture',nargs='?');p.add_argument('--port',default='COM21');p.add_argument('--baud',type=int,default=BAUD);p.add_argument('--duration',type=float,default=40);p.add_argument('--log',default='uart_p112r12_prod_auto_950plus_inert.txt');return run(p.parse_args())
