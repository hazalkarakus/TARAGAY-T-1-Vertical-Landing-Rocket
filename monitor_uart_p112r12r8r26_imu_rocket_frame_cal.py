#!/usr/bin/env python3
"""TARAGAY-T1 P112R12R8R26 IMU->rocket frame calibration + relay bench observer."""
from __future__ import annotations
import argparse, binascii, json, statistics, sys, time
from pathlib import Path

PREFIX="$TGY73,"; N=214; BAUD=115200
FIELDS=(
...
CAL_PHASE={0:"WAIT_SOURCE",1:"HOLD_UPRIGHT",2:"TILT_TOP_TO_PLUS_X",3:"HOLD_PLUS_X_TILT",4:"CALIBRATED",5:"CAL_FAULT"}
...
def serial_lines(port,baud,duration):
    ...
    with serial.Serial(port,baudrate=baud,timeout=.5) as s:
        ...
        print("="*110)
        print("R8R26 IMU -> ROCKET FRAME CALIBRATION + PHYSICAL RELAY BENCH")
        ...
        print("3) CALIBRATED gorunce dik konuma don; sonra +/-X ve +/-Y egip roleleri gozle.")
        print("="*110)
...
def status(f):
    ...
    return (f"t={iv(f,'time_ms')/1000:7.2f}s READY={iv(f,'ready')} SYS={iv(f,'system_ok')}/{iv(f,'system_fault')} "
            ...
            f"upr/tilt={iv(f,'cal_upright_samples')}/{iv(f,'cal_tilt_samples')} tilt={iv(f,'cal_tilt_cdeg')/100:.1f}deg | "
...
def run(a):
        ...
        for line in lines:
            ...
            if ph!=last_phase:
                ...
                if ph==1: print(">>> ROKETI DIK VE SABIT TUT. Yaklasik 2 saniye.")
                    ...
                    print(">>> KALIBRASYON TAMAM. DIK KONUMA DON VE +/-X +/-Y ROLE TESTINI YAP.")
                    r=matrix(f);print(">>> R_rocket_from_imu =");[print("    "," ".join(f"{x:+.4f}" for x in row)) for row in r]
                elif ph==5: print(">>> KALIBRASYON HATASI. Gucu kesmeden logu kaydet ve incele.")
                last_phase=ph
    ...
    print(f"Calibration tilt           : {iv(latest,'cal_tilt_cdeg')/100:.2f} deg")
    ...
    pass_ok=(dur>=6 and calibrated_seen and iv(latest,"cal_valid")==1 and iv(latest,"cal_fault")==0 and max_fault==0 and max_pwm==0 and delta("p111_moves")==0 and iv(latest,"system_ok")==1 and iv(latest,"eskf_ok")==1)
    print("\nSONUC:","PASS - R8R26 CALIBRATION" if pass_ok else "INCELE - calibration/gate tamamlanmadi")
...
def main():
    ...
    p.add_argument("--log",default="uart_p112r12r8r26_imu_rocket_frame_cal.txt");p.add_argument("--capture");p.add_argument("--print-every",type=int,default=3)
